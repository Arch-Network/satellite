...
The [`satellite-apl`](https://github.com/coral-xyz/anchor/tree/0e5285aecdf410fa0779b7cd09a47f235882c156/spl) crate
...
Simply add the `satellite-apl` crate as a dependency to your program and add `"satellite-apl/idl-build"` to idl-build feature list in `Cargo.toml`. For a
walkthrough of how to create an Anchor program locally, see the
[quickstart page](/docs/quickstart/local).

```shell title="Terminal"
cargo add satellite-apl
```
...
idl-build = [
    ...
    "satellite-apl/idl-build",
]

[dependencies]
satellite-lang = "0.32.0"
satellite-apl = "0.32.0"
```

### Core Modules

The most commonly used modules provided by the `satellite-apl` crate include:
...
The following pages provide examples of how to use the `satellite-apl` crate in an
Anchor program.
