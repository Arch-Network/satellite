...
impl ProgramErrorWithOrigin {
    pub fn log(&self) {
        match &self.error_origin {
            None => {
                arch_satellite_lang::arch_program::msg!(
            ...
            Some(ErrorOrigin::AccountName(account_name)) => {
                ...
                arch_satellite_lang::arch_program::log::sol_log(&format!(
        ...
        match &self.compared_values {
            Some(ComparedValues::Pubkeys((left, right))) => {
                arch_satellite_lang::arch_program::msg!("Left:");
                left.log();
                arch_satellite_lang::arch_program::msg!("Right:");
                right.log();
            }
            Some(ComparedValues::Values((left, right))) => {
                arch_satellite_lang::arch_program::msg!("Left: {}", left);
                arch_satellite_lang::arch_program::msg!("Right: {}", right);
            }
            None => (),
        }
    }
...
impl std::convert::From<Error> for arch_satellite_lang::arch_program::program_error::ProgramError {
    fn from(e: Error) -> arch_satellite_lang::arch_program::program_error::ProgramError {
        match e {
            Error::AnchorError(error) => {
                arch_satellite_lang::arch_program::program_error::ProgramError::Custom(
