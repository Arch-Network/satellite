...
pub fn gen_lazy(strct: &syn::ItemStruct) -> syn::Result<TokenStream> {
    ...
    let lazy_acc_ty = quote! { arch_satellite_lang::accounts::lazy_account::LazyAccount };
    let disc_len = quote! { <#ident as arch_satellite_lang::Discriminator>::DISCRIMINATOR.len() };
    ...
    let (loader_signatures, loader_impls) = strct
        ...
        .map(|(i, field)| {
            ...
            let ty = &field.ty;
            let ty_as_lazy = quote! { <#ty as arch_satellite_lang::__private::Lazy> };
            let size = quote! {
            ...
            let signatures = quote! {
                ...
                ///
                #load_panic_docs
                fn #load_ident(&self) -> arch_satellite_lang::Result<::core::cell::Ref<'_, #ty>>;
                ...
                ///
                #load_mut_panic_docs
                fn #load_mut_ident(&self) -> arch_satellite_lang::Result<::core::cell::RefMut<'_, #ty>>;

                 #[doc(hidden)]
                fn #load_common_ident<R>(&self, f: impl FnOnce() -> R) -> arch_satellite_lang::Result<R>;
            ...
            let impls = quote! {
                ...
                fn #load_common_ident<R>(&self, f: impl FnOnce() -> R) -> arch_satellite_lang::Result<R> {
                    ...
                    let val = arch_satellite_lang::AnchorDeserialize::try_from_slice(
    ...
    Ok(quote! {
        pub trait #lazy_ident {
            ...
            ///
            #load_panic_docs
            fn load(&self) -> arch_satellite_lang::Result<::core::cell::Ref<'_, #ident>>;
            ...
            ///
            #load_mut_panic_docs
            fn load_mut(&self) -> arch_satellite_lang::Result<::core::cell::RefMut<'_, #ident>>;
            ...
            fn exit(&self, program_id: &arch_satellite_lang::prelude::Pubkey) -> arch_satellite_lang::Result<()>;
        }
        ...
        impl<'info> #lazy_ident for #lazy_acc_ty<'info, #ident> {
            ...
            fn #load_common_ident<R>(&self, f: impl FnOnce() -> R) -> arch_satellite_lang::Result<R> {
                ...
                if all_uninit {
                    ...
                    let val = arch_satellite_lang::AnchorDeserialize::deserialize(&mut &data[offset..])?;
            ...
            fn exit(&self, program_id: &arch_satellite_lang::prelude::Pubkey) -> arch_satellite_lang::Result<()> {
                ...
                if &<#ident as arch_satellite_lang::Owner>::owner() == program_id
                    && !arch_satellite_lang::__private::is_closed(self.__info)
                {
                    // Make sure all fields are initialized
                    let acc = self.load()?;
                    let mut data = self.__info.try_borrow_mut_data()?;
                    let dst: &mut [u8] = &mut data;
                    let mut writer = arch_satellite_lang::__private::BpfWriter::new(dst);
