    ...
    let ixs = idl.instructions.iter().map(|ix| {
        ...
        let (ret_type, ret_value) = match ix.returns.as_ref() {
            Some(ty) => {
                ...
                (
                    quote! { arch_satellite_lang::Result<Return::<#ty>> },
                    quote! { Ok(Return::<#ty> { expected_program_id: ix.program_id, phantom: std::marker::PhantomData }) },
                )
            },
            None => (
                quote! { arch_satellite_lang::Result<()> },
                quote! { Ok(()) },
            )
        };
        ...
        quote! {
            pub fn #method_name<'a, 'b, 'c, 'info>(
                ctx: arch_satellite_lang::context::CpiContext<'a, 'b, 'c, 'info, accounts::#accounts_ident #accounts_generic>,
                #(#args),*
            ) -> #ret_type {
                let ix = {
                    ...
                    AnchorSerialize::serialize(&ix, &mut data)
                        .map_err(|_| arch_satellite_lang::error::ErrorCode::InstructionDidNotSerialize)?;

                    let accounts = ctx.to_account_metas(None);
                    arch_satellite_lang::arch_program::instruction::Instruction {
