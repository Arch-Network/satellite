...
pub fn generate(
        ...
        .map(|f: &AccountField| match f {
            ...
            AccountField::Field(f) => {
                ...
                if f.is_optional {
                    quote! {
                        #docs
                        pub #name: Option<arch_satellite_lang::arch_program::account::AccountInfo<'info>>
                    }
                } else {
                    quote! {
                        #docs
                        pub #name: arch_satellite_lang::arch_program::account::AccountInfo<'info>
                    }
                }
            }
        })
        ...
        .map(|f: &AccountField| match f {
            ...
            AccountField::Field(f) => {
                ...
                let meta = match f.constraints.is_mutable() {
                    false => quote! { arch_satellite_lang::arch_program::account::AccountMeta::new_readonly },
                    true => quote! { arch_satellite_lang::arch_program::account::AccountMeta::new },
                };
                let name = &f.ident;
                if f.is_optional {
                    quote! {
                        if let Some(#name) = &self.#name {
                            account_metas.push(#meta(arch_satellite_lang::Key::key(#name), #is_signer));
                        } else {
                            account_metas.push(arch_satellite_lang::arch_program::account::AccountMeta::new_readonly(#program_id, false));
                        }
                    }
                } else {
                    quote! {
                        account_metas.push(#meta(arch_satellite_lang::Key::key(&self.#name), #is_signer));
                    }
                }
            }
        })
    ...
    let account_struct_infos: Vec<proc_macro2::TokenStream> = accs
        ...
        .map(|f: &AccountField| {
            let name = &f.ident();
            quote! {
                account_infos.extend(arch_satellite_lang::ToAccountInfos::to_account_infos(&self.#name));
            }
        })
        .collect();
    ...
    quote! {
        ...
        pub(crate) mod #account_mod_name {
            ...
            impl #generics arch_satellite_lang::ToAccountMetas for #name #generics {
                fn to_account_metas(&self, is_signer: Option<bool>) -> Vec<arch_satellite_lang::arch_program::account::AccountMeta> {
            ...
            #[automatically_derived]
            impl<'info> arch_satellite_lang::ToAccountInfos<'info> for #name #generics {
                fn to_account_infos(&self) -> Vec<arch_satellite_lang::arch_program::account::AccountInfo<'info>> {
